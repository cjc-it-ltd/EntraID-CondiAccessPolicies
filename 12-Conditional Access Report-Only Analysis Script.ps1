...
function Get-CAReportOnlyAnalysis {
    ...
    Write-Host "Fetching sign-in logs for the past $DaysToAnalyze days..." -ForegroundColor Yellow
            ...
            if ($reportOnlyPolicies) {
                foreach ($policy in $reportOnlyPolicies) {
                    $resultObject = [PSCustomObject]@{
                        ...
                        Location = if ($signIn.Location.City) { "$($signIn.Location.City), $($signIn.Location.CountryOrRegion)" } else { $signIn.Location.CountryOrRegion }
...
# Get-CAReportOnlyAnalysis -Hours 12 -ReportPath "C:\CAReports"