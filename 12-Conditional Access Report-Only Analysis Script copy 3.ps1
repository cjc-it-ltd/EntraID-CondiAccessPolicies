...
function Get-CAReportOnlyAnalysis {
    ...
    param (
        [Parameter()]
        [ValidateRange(1, 168)] # Max 7 days in hours
        [int]$Hours = 7,
        ...
        [ValidateScript({
            if (!(Test-Path $_)) {
                try {
                    ...
                    throw "Cannot create directory: $_"
                }
            }
            return $true
        })]
    ...
    # Ensure output directory exists (redundant due to ValidateScript, but kept for clarity)
...
function Export-CAReportOnlyAnalysis {
    ...
    $metadata = @{
        ...
        NotAppliedCount = ($Results | Where-Object Result -eq 'reportOnlyNotApplied').Count
    }
    ...
    $byPolicy = $Results | Group-Object PolicyName | Select-Object @{
        ...
        N='Not Applied';E={($_.Group | Where-Object Result -eq 'reportOnlyNotApplied').Count}
    }
    ...
    # Define common table parameters
    $tableParams = @{
            ...
            New-HTMLPanel {
                ...
                <ul>
                    ...
                    <ul>
                        ...
                        <li>Not Applied: $($metadata.NotAppliedCount)</li>
                    </ul>
                </ul>
"@
            }
        ...
        New-HTMLSection -HeaderText "Policy Analysis" {
            New-HTMLTable -DataTable $byPolicy @tableParams {
                ...
                New-TableCondition -Name 'Not Applied' -ComparisonType number -Operator gt -Value 0 -BackgroundColor Gray -Color White
            }
        }

        New-HTMLSection -HeaderText "Application Impact" {
            New-HTMLTable -DataTable $byApp @tableParams {
        ...
        New-HTMLSection -HeaderText "User Impact" {
            New-HTMLTable -DataTable $byUser @tableParams {
                New-TableCondition -Name 'Would Block' -ComparisonType number -Operator gt -Value 0 -BackgroundColor Salmon -Color Black
            }
        }
    }
    ...
    if ($IncludeDetailedReport) {
        New-HTML -Title "Conditional Access Report-Only Analysis - Detailed Results" -FilePath $detailedHtmlPath -ShowHTML {
            New-HTMLSection -HeaderText "Detailed Results" {
                New-HTMLTable -DataTable $Results @tableParams {
                    ...
                    New-TableCondition -Name 'Result' -ComparisonType string -Operator eq -Value 'reportOnlyNotApplied' -BackgroundColor Gray -Color White
                }
            }
        }
    }
...
# Get-CAReportOnlyAnalysis -Hours 7 -UseDays -ReportPath "C:\CAReports" -IncludeDetailedReport
